...
contract('FeeCollector', async function ([currentUserAddress, wallet, wallet2]) {
    ...
    describe('LimitOrderProtocol', async function () {
        ...
        it('notifyFillOrder', async function () {
            ...
            const localFeeCollector = await FeeCollector.new(this.token.address, minValue, deceleration, currentUserAddress);

            await this.weth.mint(wallet, ether('1000000'));
            await this.weth.approve(localFeeCollector.address, ether('1000000'), { from: wallet });

            await this.token.mint(localFeeCollector.address, ether('1000'));

            await this.token.mint(wallet2, ether('1000000'));
            await this.token.approve(localFeeCollector.address, ether('1000000'), { from: wallet2 });

            await this.weth.updateReward(localFeeCollector.address, wallet, reward, { from: wallet });

            // act
            const balancesBeforeFill = (await localFeeCollector.getTotalSupplyEpochBalance(this.weth.address, 0))
                .add(await localFeeCollector.getTotalSupplyEpochBalance(this.weth.address, 1));
            await localFeeCollector.notifyFillOrder(localFeeCollector.address, this.token.address, 10, 5, this.weth.address);
            const balancesAfterFill = (await localFeeCollector.getTotalSupplyEpochBalance(this.weth.address, 0))
                .add(await localFeeCollector.getTotalSupplyEpochBalance(this.weth.address, 1));

            expect(balancesBeforeFill).to.be.bignumber.equal(balancesAfterFill);
        });
    });
    ...
    describe('valueForTime', async function () {
        ...
        it('_updateTokenState', async function () {
            ...
            let price = await this.feeCollector.value.call(this.weth.address);
            ...
            price = await this.feeCollector.value.call(this.weth.address);
    ...
    describe('updateReward', async function () {
        ...
        it('tokenInfo after the freezing epoch', async function () {
            ...
            const price = await this.feeCollector.value.call(this.weth.address);
            const tokenInfo1 = await getTokenInfo(this.feeCollector, this.weth.address, wallet, 0);
    ...
    describe('trade', async function () {
        ...
        it('tokenInfo after trade frozen and current epoch', async function () {
            ...
            const price2 = await this.feeCollector.value.call(this.weth.address);
